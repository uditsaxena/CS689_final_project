...
class Video_Caption_Generator():
    ...
    def build_model(self):
        video = tf.placeholder(tf.float32, [self.batch_size, self.n_lstm_steps, self.dim_image],name = "video_placeholder_model")
        video_mask = tf.placeholder(tf.float32, [self.batch_size, self.n_lstm_steps],name = "video_mask_placeholder_model")

        caption = tf.placeholder(tf.int32, [self.batch_size, self.n_lstm_steps],name = "caption_placeholder_model")
        caption_mask = tf.placeholder(tf.float32, [self.batch_size, self.n_lstm_steps],name = "caption_mask_placeholder_model")
...
def test(model_path='models/model-900', video_feature_path=video_feat_path):
